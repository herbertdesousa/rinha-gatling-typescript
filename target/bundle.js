var gatling = (() => {
  ...
  var require_feeders = __commonJS({
    "node_modules/@gatling.io/core/target/feeders.js"(exports) {
      ...
      exports.ssv = ssv;
      var tsv2 = (filePath, quoteChar) => wrapBatchableFeederBuilder(quoteChar !== void 0 ? jvm_types_1.CoreDsl.tsv(filePath, quoteChar) : jvm_types_1.CoreDsl.tsv(filePath));
      exports.tsv = tsv2;
  ...
  var require_openInjection = __commonJS({
    "node_modules/@gatling.io/core/target/openInjection.js"(exports) {
      ...
      var rampUsersPerSec2 = (rate) => wrapOpenInjectionStepRampRate(jvm_types_1.CoreDsl.rampUsersPerSec(rate));
      exports.rampUsersPerSec = rampUsersPerSec2;
  ...
  var require_parameters = __commonJS({
    "node_modules/@gatling.io/core/target/parameters.js"(exports) {
      ...
      var jvm_types_1 = require_jvm_types();
      var getOption = (key, defaultValue) => getOrElse(jvm_types_1.System.getProperty(key), defaultValue);
      exports.getOption = getOption;
      var getEnvironmentVariable2 = (name, defaultValue) => getOrElse(jvm_types_1.System.getenv(name), defaultValue);
      exports.getEnvironmentVariable = getEnvironmentVariable2;
  ...
  var require_checks2 = __commonJS({
    "node_modules/@gatling.io/http/target/checks.js"(exports) {
      ...
      var header2 = (name) => (0, core_1.wrapCheckBuilderMultipleFind)(typeof name === "function" ? jvm_types_1.HttpDsl.header((0, core_1.underlyingSessionTo)(name)) : jvm_types_1.HttpDsl.header(name));
  ...
  __export(stdin_exports, {
    rinhadebackend: () => rinhadebackend_gatling_default
  });

  // src/rinhadebackend.gatling.ts
  var import_core = __toESM(require_target(), 1);
  var import_http = __toESM(require_target2(), 1);
  var rinhadebackend_gatling_default = (0, import_core.simulation)((setUp) => {
    const httpProtocol = import_http.http.baseUrl((0, import_core.getEnvironmentVariable)("TEST_URL", "http://192.168.15.12")).userAgentHeader("Agente do Caos - 2023");
    const criacaoEConsultaPessoas = (0, import_core.scenario)("Cria\xE7\xE3o E Talvez Consulta de Pessoas").feed((0, import_core.tsv)("pessoas-payloads.tsv").circular()).exec(
      (0, import_http.http)("cria\xE7\xE3o").post("/pessoas").body((0, import_core.StringBody)("#{payload}")).header("content-type", "application/json").check((0, import_http.status)().in(201, 422, 400)).checkIf((session) => session?.httpStatus === 201).then((0, import_http.header)("Location").saveAs("location"))
      // .check(status().in(201.502))
      // .checkIf(session => (session as any)?.httpStatus === 201)
      // .then(session => console.log())
    ).pause(1, 30).doIf((session) => session.contains("location")).then((0, import_core.exec)((0, import_http.http)("consulta").get((session) => session.location))).doIf((session) => session.contains("location")).then((0, import_core.exec)((0, import_http.http)("http://localhost:3333").post((session) => session.location)));
    const buscaPessoas = (0, import_core.scenario)("Busca V\xE1lida de Pessoas").feed((0, import_core.tsv)("termos-busca.tsv").circular()).exec(
      (0, import_http.http)("busca v\xE1lida").get("/pessoas?t=#{t}")
      // qq resposta na faixa 2XX tá safe
    );
    const buscaInvalidaPessoas = (0, import_core.scenario)("Busca Inv\xE1lida de Pessoas").exec(
      (0, import_http.http)("busca inv\xE1lida").get("/pessoas").check((0, import_http.status)().in(400))
    );
    setUp(
      criacaoEConsultaPessoas.injectOpen(
        (0, import_core.constantUsersPerSec)(2).during({ amount: 10, unit: "seconds" }),
        // warm up
        (0, import_core.constantUsersPerSec)(5).during({ amount: 15, unit: "seconds" }).randomized(),
        // are you ready?
        (0, import_core.rampUsersPerSec)(6).to(600).during({ amount: 3, unit: "minutes" })
        // lezzz go!!!
      ),
      buscaPessoas.injectOpen(
        (0, import_core.constantUsersPerSec)(2).during({ amount: 25, unit: "seconds" }),
        // warm up
        (0, import_core.rampUsersPerSec)(6).to(100).during({ amount: 3, unit: "minutes" })
        // lezzz go!!!
      ),
      buscaInvalidaPessoas.injectOpen(
        (0, import_core.constantUsersPerSec)(2).during({ amount: 25, unit: "seconds" }),
        // warm up
        (0, import_core.rampUsersPerSec)(6).to(40).during({ amount: 3, unit: "minutes" })
        // lezzz go!!!
      )
